...
func AddScan(scantype string, info structs.HostInfo, ch *chan struct{}, wg *sync.WaitGroup) {
	count := atomic.AddInt32(&currentCount, 1)
	if count%100 == 0 {
		gologger.Info().Msgf("[GoPoc] 当前进度: %v %v [%v/%v]", scantype, info.Host+":"+info.Ports, count, allCount)
	}
...
func ScanFunc(name *string, info *structs.HostInfo) {
	...
	if !exists {
		gologger.Warning().Msgf("[-] Plugin not found: %v\n", *name)
		return
	}
